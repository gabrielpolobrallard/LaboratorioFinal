...
{
    public partial class PacientesNewForm2 : Base
    {
        DataSet ds=new DataSet();
        private int iniciando;
        public PacientesNewForm2()
        {
            InitializeComponent();
            iniciando = 1;
            cargarDataGrid();
            MessageBox.Show("1: "+ds.Tables.Count.ToString());
            cargarComboSang();
            MessageBox.Show("2: " + ds.Tables.Count.ToString());
        ...
        {
            string conStr = "Data Source=JAVA-B2S1EMXNU5;Initial Catalog=LabDb;Integrated Security=yeS;User id=sa;Password=admin";
            SqlConnection conn = new SqlConnection(conStr);
            // Open the connection
            conn.Open();


            SqlCommand command = new SqlCommand("getGruposSanguineosAll", conn);
        ...
        private void cargarDataSetPacientes()
        {
            ...
            SqlCommand command = new SqlCommand("getPacientesTodos", conn);
            command.CommandType = CommandType.StoredProcedure;
            //command.Parameters.Add("@Nocontrol", SqlDbType.NVarChar).Value = this.txt_nocontrol.Text;
            SqlDataAdapter adapters = new SqlDataAdapter( command );
           //  ds = new DataSet();
